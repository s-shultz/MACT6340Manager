...
export async function buildFinalMetaAndPinToIPFS() {
    ...
    for (let i = 0; i < projectInfo.numberOfEditions; i++) {
        let tokenId = i + 1; //getTokenId(i);
        let finalMeta = `{\n  "image": "${JSON.parse(imageIPFS[i]).ipfs
            }",\n  "background_color": "96231B",\n  "external_url": "${projectInfo.projectWebUrl
...
export async function buildProjectMetaAndPinToIPFS() {
    ...
    let projectMeta =
        "" +
        `{"name": "${projectInfo.openSeaCollectionName}","description": "${projectInfo.openSeaCollectionDescription
        }","image": "${JSON.parse(imageIPFS[0]).ipfs}","external_link": "${projectInfo.projectWebUrl
